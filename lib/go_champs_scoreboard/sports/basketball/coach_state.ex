defmodule GoChampsScoreboard.Sports.Basketball.CoachState do
  ...
  def update_coach_state(coach) do
    ...
    disqualifying_fouls = Map.get(coach.stats_values, "fouls_disqualifying", 0)
    disqualifying_fighting_fouls = Map.get(coach.stats_values, "fouls_disqualifying_fighting", 0)
    current_state = Map.get(coach, :state, :available)

    if (fouls >= 3 or game_disqualifying_fouls >= 1 or disqualifying_fouls >= 1 or
          disqualifying_fighting_fouls >= 1) and
         current_state != :disqualified do
